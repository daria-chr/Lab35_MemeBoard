...
public class MemesController : ControllerBase {
    ...
    public ActionResult<Meme> Create([FromBody] Meme meme) {
        if (string.IsNullOrWhiteSpace(meme.Title)) {
            return BadRequest(new { message = "Назыание мема не может быть пустым" });
        }
        if (meme.Rating < 1 || meme.Rating > 5) {
            return BadRequest(new { message = "Рейтинг должен от 1 до 5" });
        }
    ...
    public ActionResult Delete(int id) {
        var meme = MemesStore.Memes.FirstOrDefault(m => m.Id == id);
        if (meme is null) {
            return NotFound(new { messahe = $"Мем с id={id} не найден" });
        }
